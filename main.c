...
{	//변수 선언 영역 
	...
	//int suspect_ithPlace; //대상환자의 i번째 장소  => 필요없는 듯 
	int infectee_ithPlace; //현재환자의 i번째 장소 
	int suspect_infestedTime; //대상환자의 감염시점  
	
	int ithTime; //현재환자의 i번째 장소 당시 시점 
	int kthTime; //대상환자의 k번째 장소 당시 시점  
	...
	for (i=0; i<3; i++) //현재환자가 감염될 수 있는 시점동안 반복 ( [0], [1], [2] ) 
	{
		...
		for (k= 3; k<N_HISTORY;k++) //대상환자가 감염시킬 수 있는 시점동안 반복 ( [3], [4] ) 
		{
			...
			if ( infectee_ithPlace == suspect_kthPlace )	//같은 장소에 있던 것이 맞다면(현재환자의 i번째 장소 == 대상환자의 k번째 장소인지) => //같은 시점인지 비교 
			{
				...
				if (ithTime ==kthTime)//시점도 같다면 (현재환자 i번째 장소 시점 == 대상환자 k번째 시점) 
				{
				 	metTime_isMet = ithTime ; //만난 시간==ithTime ==kthTime (초기값인 -1에서 현재환자 i번째 장소시점(=대상환자 k시점과 같음)으로 변경 
				} 
			}
		}
	}	 
